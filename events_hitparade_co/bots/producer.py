...
class HitParadeProducerBot(HitParadeBot):
    ...
    def poststate(self, **kwargs):
        ...
        if not v.get('current_url', None) is None and ('http://' in  v.get('current_url', None) or 'https://' in v.get('current_url', None)):
            ...
            v['collection'] = self.state_storage_get_prop('data_selectors').get('collection', None)
            v['database'] = self.state_storage_get_prop('data_selectors').get('database', None)
            v['reformatter'] = self.state_storage_get_prop('data_selectors').get('reformatter', None)
            v['database_serializer'] = self.state_storage_get_prop('data_selectors').get('database_serializer', None)
